...
def test_validatecli_valid(tmpdir):
    runner = CliRunner()
    result = runner.invoke(packtivity.cli.validatecli,['tests/testspecs/noop-test.yml'])
    assert result.exit_code == 0

def test_validatecli_invalid(tmpdir):
    runner = CliRunner()
    result = runner.invoke(packtivity.cli.validatecli,['tests/testspecs/noop-test-invalid.yml'])
