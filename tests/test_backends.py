...
def test_known():
    for known_backend in ['celery', 'multiproc:4', 'multiproc:auto', 'foregroundasync','externalasync:default']:
        b = backend_from_string(known_backend)
        assert b

def test_python_import():
    b = backend_from_string('py:packtivity.asyncbackends:MultiProcBackend', {'poolsize': 1})
    assert b
